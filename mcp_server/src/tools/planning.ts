...
export const planningTools = [
...

  {
    ...
    inputSchema: {
      type: "object" as const,
      properties: {
        moduleId: { type: "string", description: "The module's unique identifier" },
        updates: {
          type: "array",
          description: "Array of trackable updates — each item targets an existing trackable by ID",
          items: {
            type: "object",
            properties: {
              id: { type: "string", description: "The trackable's unique identifier" },
              data: { description: "The new data value to store for this trackable" },
            },
            required: ["id", "data"],
          },
        },
...

  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        limit: {
          type: "number",
          description: "Maximum number of states to return (default 7, max 90)",
        },
      },
      required: ["userId"],
    },
    handler: async (args: { userId: string; limit?: number }) => {
      ...
      const states = await prisma.state.findMany({
        ...
        take: limit,
        include: {
          modules: { select: { id: true, type: true, summary: true } },
        },
      });
