  ...
  gl_FragColor = vec4(mix(ground, u_accent, dotAmt), tex.a);
    ...
    function renderColorTiles(body, colors) {
        for (const c of colors) {
            ...
            meta.innerHTML = `<span class="name">${escapeHtml(
                c.name || c.role || "Color",
            )}</span><span>${escapeHtml(c.value || "")}</span>`;
    ...
    function renderTypeTiles(body, types) {
        for (const t of types) {
            ...
            meta.innerHTML = `<span>${escapeHtml(
                t.role || "",
            )}</span><span>${escapeHtml(t.weight || "")} ${escapeHtml(
    ...
    function renderComponentTiles(body, components) {
        ...
        for (const group of groups) {
            ...
            meta.innerHTML = `<span class="name">${escapeHtml(
                groupTitle,
            )}</span><span class="cmp-kind">${escapeHtml(
                group[0].kind || "",
            )}</span>`;
            tile.appendChild(meta);
    ...
    function buildCollapsible(key, label, count) {
      ...
      ${
          count != null
              ? `<span class="coll-count">${escapeHtml(String(count))}</span>`
              : ""
      }
    `;
