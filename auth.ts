                ...
                if (!token.role) {
                    ...
                    if (dbUser) {
                        ...
                        if (dbUser.role === "admin") {
                            token.adminManagedPatientIds =
                                dbUser.adminManagedPatients.map(
                                    (rel) => rel.patientId,
                                );
                        }
                    }
        ...
        async session({ session, token }) {
            if (session.user) {
                ...
                try {
                    ...
                    console.error(
                        "[SESSION] Failed to fetch doneOnboarding state:",
                        error,
                    );
                    session.user.doneOnboarding =
                        token.doneOnboarding as boolean;
                }

                // Expose admin managed patient IDs if user is admin
                if (token.role === "admin" && token.adminManagedPatientIds) {
                    session.user.adminManagedPatientIds =
                        token.adminManagedPatientIds as string[];
                }
            }
            return session;
        },
