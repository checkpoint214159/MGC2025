...
(function () {
        ...
        if (hasDirectText && textColor && !isEmojiOnly) {
            ...
            if (bgs) {
                ...
                if (ratio < threshold) {
                    ...
                    if (!isAlphaFallbackFP) {
                        findings.push({
                            id: "low-contrast",
                            snippet: `${ratio.toFixed(
                                1,
                            )}:1 (need ${threshold}:1) — text ${colorToHex(
                                textColor,
                            )} on ${colorToHex(bgs[worstIdx])}`,
                        });
                    }
                }
            }
            ...
            if (hasChroma(textColor, 50)) {
                ...
                if (
                    ...
                    findings.push({
                        id: "ai-color-palette",
                        snippet: `Purple/violet text (${colorToHex(
                            textColor,
                        )}) on heading`,
                    });
                }
        ...
        return [
            {
                ...
                snippet: `${Math.round(siblingWidth)}x${Math.round(
                    siblingHeight,
                )}px icon tile above ${headingTag} "${text}"`,
            },
        ];
    }
    ...
    function checkItalicSerif(opts) {
        ...
        return [
            {
                id: "italic-serif-display",
                snippet: `italic serif ${tag} (${
                    primary || "serif"
                }) at ${Math.round(fontSize)}px "${text}"`,
            },
        ];
    }
        ...
        for (const shadow of parts) {
            ...
            if (pxVals.length >= 3 && pxVals[2] > 4) {
                return [
                    {
                        id: "dark-glow",
                        snippet: `Colored glow (${colorToHex(
                            color,
                        )}) on dark background`,
                    },
                ];
    ...
    function checkHtmlPatterns(html) {
        ...
        if (roundedSpacing.length >= 10) {
            ...
            if (dominantPct > 0.6 && unique.length <= 3) {
                ...
                findings.push({
                    id: "monotonous-spacing",
                    snippet: `~${dominant}px used ${maxCount}/${
                        roundedSpacing.length
                    } times (${Math.round(dominantPct * 100)}%)`,
                });
            }
        }
        ...
        if (bounceMatch) {
            ...
            findings.push({
                id: "bounce-easing",
                snippet: `animation: ${
                    animationToken || bounceMatch[1].trim()
                }`,
            });
        }
        ...
        if (
            ...
            if (lineHeightPx != null && fontSize > 0) {
                ...
                if (ratio > 0 && ratio < 1.3) {
                    findings.push({
                        id: "tight-leading",
                        snippet: `line-height ${ratio.toFixed(
                            2,
                        )}x (need >=1.3)`,
                    });
                }
    ...
    function checkPageQualityFromDoc(doc) {
        ...
        for (const h of headings) {
            ...
            if (prevLevel > 0 && level > prevLevel + 1) {
                findings.push({
                    id: "skipped-heading",
                    snippet: `<h${prevLevel}> "${prevText}" followed by <h${level}> "${text}" (missing h${
                        prevLevel + 1
                    })`,
                });
            }
    ...
    function checkTypography() {
        ...
        if (totalTextElements >= 20) {
            ...
            for (const [font, count] of fontUsage) {
                ...
                findings.push({
                    type: "overused-font",
                    detail: `Primary font: ${font} (${Math.round(
                        share * 100,
                    )}% of text)`,
                });
            }
            ...
            return [
                {
                    ...
                    snippet: `${Math.round(
                        fontSize,
                    )}px h1, ${textLen} chars${viewportDetail} "${headingText.slice(
                        0,
                        60,
                    )}"`,
                },
    ...
    function checkGptThinBorderWideShadow({
        ...
        if (visibleThinBorders.length >= 2 && blur >= 16) {
            return [
                {
                    id: "gpt-thin-border-wide-shadow",
                    snippet: `${maxBorder}px border + ${Math.round(
                        blur,
                    )}px shadow blur`,
                },
            ];
    ...
    function positionedChildIsDecorative(child) {
        ...
        const ident = `${child.getAttribute("class") || ""} ${
    ...
    function clippingContainerIsIntentionalViewport(el) {
        ...
        const ident = `${el.getAttribute("class") || ""} ${
            el.getAttribute("id") || ""
        }`.toLowerCase();
        return (
            ...
            return [
                {
                    ...
                    snippet: `${classSelector(
                        el,
                    )} overflows its box by ${Math.round(delta)}px`,
                },
            ];
        }
        return [];
    ...
    .impeccable-hidden .impeccable-overlay${
        EXTENSION_MODE ? "" : ":not(.impeccable-banner)"
    } {
  ...
  `;
        ...
        function updateSpotlightClipPath() {
            ...
            const path = `M0 0H${vw}V${vh}H0Z M${x1 + radius} ${y1}H${
                x2 - radius
            }A${radius} ${radius} 0 0 1 ${x2} ${y1 + radius}V${
                y2 - radius
            }A${radius} ${radius} 0 0 1 ${x2 - radius} ${y2}H${
                x1 + radius
            }A${radius} ${radius} 0 0 1 ${x1} ${y2 - radius}V${
                y1 + radius
            }A${radius} ${radius} 0 0 1 ${x1 + radius} ${y1}Z`;
        ...
        const showPageBanner = function (findings) {
            ...
            for (const f of findings) {
                ...
                tag.textContent = `${prefix}${TYPE_LABELS[f.type] || f.type}: ${
                    f.detail
                }`;
        ...
        async function analyzeVisualContrastCandidate(candidate) {
            ...
            const detail = `browser contrast ${measuredRatio.toFixed(
                1,
            )}:1 median ${medianRatio.toFixed(1)}:1 (need ${
                candidate.threshold
            }:1) via ${method}${textLabel}`;
            return {
                ...candidate,
                status,
        ...
        const printSummary = function (allFindings) {
            ...
            console.group(
                `%c[impeccable] ${allFindings.length} anti-pattern${
                ...
                if (
                    ...
                    findings.push({
                        ...
                        detail: `${tag}${browserSampleText(
                            el,
                        )} uses ${font}; not declared in DESIGN.md typography`,
                        ignoreValue: font,
                    });
                }
            }
                ...
                for (const token of browserRadiusTokens(
                    ...
                    findings.push({
                        ...
                        detail: `border-radius ${token} on ${tag}${browserSampleText(
                            el,
                        )} is outside the DESIGN.md rounded scale`,
                        ignoreValue: token,
                    });
                }
            }
