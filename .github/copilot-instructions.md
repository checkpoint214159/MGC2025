...
-   **LLM logic** is central; there are two LLM subsystems:
    -   _Onboarding/questionnaire_ in `lib/llm/` (`service.ts`, `model.ts`, `schemas/`). Uses `generateObject()` from `ai` + Zod schemas to request and validate structured questions/responses.
    -   _State generation_ (see `lib/state/` and `lib/state/services/full.ts`). A "blueprint" approach produces daily modules (nutrition, exercise, etc.) from previous state + compiled external context.
