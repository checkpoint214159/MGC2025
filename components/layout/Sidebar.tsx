...
const NAV: NavItem[] = [
    ...
    {
        href: "/patient/dashboard?log=exercise",
        label: "Exercise",
        icon: Activity,
    },
    {
        href: "/patient/dashboard?log=nutrition",
        label: "Nutrition",
        icon: Salad,
    },
...
export default function Sidebar() {
    ...
    const onDashboard =
        pathname.startsWith("/patient/dashboard") || pathname === "/";
    function isActive(href: string): boolean {
        const [base, query] = href.split("?");
        if (query?.startsWith("log=")) {
            return onDashboard && log === query.slice("log=".length);
        }
        if (base === "/") return onDashboard && !log;
            ...
            <div className="px-3 pb-4 pt-2 border-t border-border space-y-0.5">
                {!collapsed && (
                    <div className="px-1 pb-2">
                        <div className="mb-1.5 text-[12px] font-medium text-ink-subtle">
                            Text size
                        </div>
                        <div className="flex gap-1">
                            {TEXT_SCALES.map((s, i) => (
                                ...
                                >
                                    <span style={{ fontSize: [13, 15, 18][i] }}>
                                        A
                                    </span>
                                </button>
                            ))}
