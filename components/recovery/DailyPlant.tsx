    ...
    bloom: (cx, cy) => {
        ...
        return (
            <g>
                {/* outer side petals */}
                <path
                    d={`M ${cx} ${b} C ${cx - 12} ${cy + 5} ${cx - 11} ${
                        cy - 10
                    } ${cx - 3} ${cy - 11} C ${cx - 2} ${cy - 2} ${cx} ${
                        cy + 3
                    } ${cx} ${b} Z`}
                    fill="oklch(0.58 0.2 24)"
                    stroke="oklch(0.5 0.2 24)"
                    strokeWidth={0.7}
                />
                <path
                    d={`M ${cx} ${b} C ${cx + 12} ${cy + 5} ${cx + 11} ${
                ...
                {/* front centre petal */}
                <path
                    d={`M ${cx} ${b} C ${cx - 6} ${cy + 2} ${cx - 5.5} ${
                        cy - 12
                    } ${cx} ${cy - 13} C ${cx + 5.5} ${cy - 12} ${cx + 6} ${
                        cy + 2
                    } ${cx} ${b} Z`}
                    fill="oklch(0.66 0.21 26)"
                    stroke="oklch(0.52 0.2 24)"
                    strokeWidth={0.7}
                />
                <path
                    d={`M ${cx} ${cy - 9} L ${cx} ${cy + 6}`}
                    stroke="oklch(0.54 0.2 24)"
                    strokeWidth={0.6}
                    opacity={0.5}
                />
            </g>
    ...
    bloom: (cx, cy, { petals }) => (
        <g>
            ...
            <circle
                ...
                cy={cy}
                r={3.4}
                fill="oklch(0.74 0.15 8)"
                stroke="oklch(0.58 0.18 10)"
                strokeWidth={0.6}
            />
            <path
                d={`M ${cx} ${cy} m -2.4 0 a 2.4 2.4 0 1 1 4.8 0.6 a 1.5 1.5 0 1 1 -3 -0.2`}
                fill="none"
...
export function PlantArt({
    ...
    const stemD = `M ${X - 2.6} ${SOIL} L ${X - 1.3} ${top + 2} Q ${X} ${top} ${
        X + 1.3
    } ${top + 2} L ${X + 2.6} ${SOIL} Z`;
    ...
    return (
        <svg
            ...
            className={className}
            role="img"
            aria-label={`${sp.label} — ${STAGE_LABEL[stage]}`}
        >
            <defs>
            ...
            <ellipse
                ...
                rx={28}
                ry={4}
                fill="oklch(0.55 0.03 150)"
                opacity={0.18}
            />

            {/* pot — chunky, rounded, flat two-tone clay */}
            <path
                d={`M 37 ${SOIL} L 41 ${SOIL + 26} Q 41.5 ${SOIL + 31} 47 ${
                    SOIL + 31
                } L 73 ${SOIL + 31} Q 78.5 ${SOIL + 31} 79 ${
                    SOIL + 26
                } L 83 ${SOIL} Z`}
                fill="oklch(0.66 0.1 48)"
            />
            <path
                d={`M 60 ${SOIL} L 60 ${SOIL + 31} L 73 ${SOIL + 31} Q 78.5 ${
                    SOIL + 31
                } 79 ${SOIL + 26} L 83 ${SOIL} Z`}
                fill="oklch(0.6 0.105 45)"
            />
            {/* pot rim */}
            <rect
                x={33}
            ...
            <rect
                ...
                width={27}
                height={9}
                fill="oklch(0.64 0.1 47)"
            />
            {/* soil cap */}
            <path
                d={`M 35.5 ${SOIL - 1} Q ${X} ${SOIL - 6} 84.5 ${
                    SOIL - 1
                } L 84.5 ${SOIL} L 35.5 ${SOIL} Z`}
                fill="oklch(0.42 0.045 62)"
            />
            {[
                [-13, -1.5],
                [-4, -3],
            ...
            {stage === 0 && (
                <ellipse
                    ...
                    rx={4.5}
                    ry={3}
                    transform={`rotate(-18 ${X} ${SOIL - 3.5})`}
                    fill="oklch(0.48 0.06 62)"
                    stroke="oklch(0.38 0.05 60)"
                    strokeWidth={0.8}
                />
            )}
            ...
            {stage === 5 && (
                <g>
                    <path
                        d={`M ${X} ${top + 4} Q ${X - 7} ${top - 6} ${X} ${
                            top - 14
                        } Q ${X + 7} ${top - 6} ${X} ${top + 4} Z`}
                        fill="oklch(0.56 0.14 148)"
                    />
                    <path
                        d={`M ${X} ${top - 14} Q ${X - 4} ${top - 9} ${X} ${
                            top - 2
                        } Q ${X + 4} ${top - 9} ${X} ${top - 14} Z`}
                        fill={sp.tip}
                    />
                </g>
            )}
...
function Leaf({
    ...
    return (
        <g transform={`translate(${x} ${y}) rotate(${rot})`}>
            {/* lower (darker) half */}
            <path
                d={`M 0 0 C ${len * 0.06} ${w} ${tipCtl} ${w} ${len} 0 Z`}
                fill="oklch(0.5 0.14 147)"
            />
            {/* upper (lighter) half */}
            <path
                d={`M 0 0 C ${len * 0.06} ${-w} ${tipCtl} ${-w} ${len} 0 Z`}
                fill="oklch(0.66 0.14 150)"
            />
            {/* midrib */}
            <path
                d={`M 1.5 0 L ${len - 1.5} 0`}
                stroke="oklch(0.46 0.13 146)"
                strokeWidth={0.7}
                strokeLinecap="round"
                opacity={0.7}
            />
            ...
            <path
                d={`M 0 0 C ${
                    len * 0.06
                } ${-w} ${tipCtl} ${-w} ${len} 0 C ${tipCtl} ${w} ${
                    len * 0.06
                } ${w} 0 0 Z`}
                fill="none"
                stroke="oklch(0.43 0.13 146)"
                strokeWidth={0.8}
            />
        </g>
    );
...
export function DailyPlant({ done, total }: { done: number; total: number }) {
    const pct =
        total > 0 ? Math.min(Math.max((done / total) * 100, 0), 100) : 0;
    ...
    const msg =
        stage === 6
            ? `In full bloom — a ${flower} for the whole day.`
            : stage >= 4
              ? "Growing nicely. Keep going."
              : stage >= 1
        ...
        <div className="flex items-center gap-6 rounded-xl border border-border bg-surface p-7 md:p-8">
            ...
            <div className="flex-1">
                ...
                <div className="mt-3 h-2 w-full max-w-xs overflow-hidden rounded-full bg-surface-sunken">
                    <div
                        className="h-full rounded-full bg-progress transition-[width] duration-500 ease-[var(--ease-out-quart)]"
                        style={{ width: `${pct}%` }}
                    />
                </div>
            </div>
