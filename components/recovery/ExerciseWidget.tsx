...
export default function RecoveryExerciseRenderer({
    ...
    const theme =
        INTENSITY_THEMES[intensity as keyof typeof INTENSITY_THEMES] ??
        INTENSITY_THEMES.blue;
    const { isCaregiver } = useCaregiver();

    const trackableId = trackable.id;
    const [localValues, setLocalValues] = useState<Record<string, number>>(() =>
        Object.fromEntries(
            Object.entries(trackable.data).map(
                ([k, m]: [string, ExerciseMetrics]) => [k, m.value],
            ),
        ),
    );
    ...
    const hasChanges = useMemo(
        () =>
            Object.entries(localValues).some(([k, v]) => {
                const trkData = trackable.data as Record<
                    string,
                    ExerciseMetrics
                >;
            ...
            <Card className="space-y-5">
                <div className="flex items-start justify-between gap-3">
                    <div className="space-y-1.5">
                        <h3 className="text-[17px] font-semibold text-ink capitalize leading-tight">
                            {name}
                        </h3>
                        <Chip
                            tone={
                ...
                <div className="space-y-5">
                    {Object.entries(plan.data).map(
                        ([key, metric]: [string, ExerciseMetrics]) => {
                            const current = localValues[key] || 0;
                            return (
                                <div key={key} className="space-y-2">
                                    <div className="flex items-end justify-between">
                                        <div className="space-y-0.5">
                                            <p className="text-[13px] font-medium text-ink-muted capitalize">
                                                {key}
                                            </p>
