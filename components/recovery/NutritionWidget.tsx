...
export default function NutritionWidget({ plan, trackable, moduleId }: Props) {
    ...
    const [localValues, setLocalValues] = useState<Record<string, number>>(() =>
        Object.fromEntries(
            Object.entries(trkData).map(([k, v]) => [k, v.value || 0]),
        ),
    );
    ...
    const hasChanges = useMemo(
        () =>
            Object.entries(localValues).some(
                ([k, v]) => v !== trkData[k]?.value,
            ),
        [localValues, trkData],
    );
    ...
    async function onSave() {
        ...
        const updated = Object.fromEntries(
            Object.entries(trkData).map(([k, v]) => [
                k,
                { ...v, value: localValues[k] },
            ]),
        );
        try {
            await updateProgressAction(moduleId, [
        ...
        <Card className="space-y-5">
            <div className="flex items-start justify-between">
                <div className="space-y-1.5">
                    <h3 className="text-[17px] font-semibold text-ink capitalize">
                        {meta.type || type}
                    </h3>
