...
export function TaskFlow({
  ...
  return (
    <Card>
      ...
      {phase === "idle" && (
        <Button className="mt-4" size="lg" onClick={() => setPhase("doing")}>
      ...
      {phase === "doing" && (
        <div className="mt-4 space-y-3">
          <div className="rounded-md bg-surface-sunken p-4 text-[15px] text-ink-muted">{guidance}</div>
      ...
      {phase === "aar" &&
        (comfort ? (
          <p className="mt-4 inline-flex items-center gap-1.5 text-[14px] text-progress-ink">
            <Check size={15} strokeWidth={2.5} /> Logged — “{comfort}”. Nicely done.
          </p>
        ) : (
          <div className="mt-4 space-y-2">
            <p className="text-[15px] text-ink">How did that feel?</p>
            <div className="flex flex-wrap gap-2">
              {["Comfortable", "A bit hard", "Too much"].map((c, i) => (
                <Button
                  key={c}
                  size="md"
