...
export default function SymptomPreviewCard({ data, onClick }: { data: any, onClick: () => void }) {
  return (
    <DashboardCard
    ...
    >
      <div className="flex items-center gap-4">
        <div className="flex flex-col">
          ...
          <span className="text-lg font-black text-slate-900">Low (2/10)</span>
        </div>
        <div className="h-8 w-[1px] bg-slate-100" />
        <div className="flex flex-col">
          <span className="text-[10px] font-bold text-slate-400 uppercase">Primary Site</span>
          <span className="text-sm font-bold text-slate-700">Right Knee</span>
        </div>
      </div>
    </DashboardCard>
  );
