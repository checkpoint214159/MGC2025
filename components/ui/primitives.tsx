...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
    function Button(
        ...
        const variants: Record<ButtonVariant, string> = {
            primary:
                "bg-accent text-ink-inverse hover:bg-accent-hover active:translate-y-[0.5px]",
            secondary:
...
export function Stat({
    ...
    return (
        <div className={cn("flex flex-col gap-0.5", className)}>
            <span className="text-[12px] font-medium text-ink-muted uppercase tracking-wide">
                {label}
            </span>
            <span className="text-[20px] font-semibold text-ink leading-tight tabular-nums">
                {value}
            </span>
    ...
    return (
        <div className="w-full">
            ...
            >
                <div
                    className={cn(
                        "h-full transition-[width] duration-500 ease-[var(--ease-out-quart)] rounded-full",
                        tones[tone],
                    )}
                    style={{ width: `${pct}%` }}
                />
        ...
        <div className="flex items-center gap-3">
            ...
            >
                <svg
                    ...
                    aria-label={
                        atCap
                            ? "7 of 7 day streak — well held"
                            : `${count} of 7 day streak`
                    }
                >
                    {Array.from({ length: segments }).map((_, i) => {
                        ...
                        return (
                            <circle
                                ...
                                fill="none"
                                stroke={
                                    filled ? "var(--progress)" : "var(--border)"
                                }
                                strokeWidth={stroke}
                                strokeLinecap={atCap ? "butt" : "round"}
                                strokeDasharray={`${segLen} ${
                                    circumference - segLen
                                }`}
                                strokeDashoffset={-offset}
                                style={{
                                    transition:
                                        "stroke 600ms var(--ease-out-quart)",
                                }}
                            />
                        );
                    })}
    ...
    return (
        <div className="mb-4 flex items-center gap-2.5 rounded-md bg-accent-soft px-4 py-2.5 text-[13px] text-accent-ink">
            ...
            <span className="flex-1">
                Caregiver view — read-only. Helping{" "}
