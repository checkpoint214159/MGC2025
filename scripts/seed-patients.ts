...
async function main() {
    ...
    if (args.cleanup) {
        ...
        if (confirm !== "yes") {
            console.log(
                "Cleanup cancelled. Set FORCE_CLEANUP=true to force deletion.\n",
            );
            process.exit(0);
        }
    ...
    if (args.count && args.count > 0) {
        console.log(
            `\n🌱 Seeding ${args.count} patients with preset "${
                args.preset || "colostomy-default"
            }"...\n`,
        );
