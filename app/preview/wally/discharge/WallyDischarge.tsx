            ...
            {doc.uploaded ? (
                <div className="rounded-xl border border-progress/30 bg-progress-soft/40 p-3">
                    <div className="flex items-center gap-3">
                        <FileText
                            size={22}
                            className="shrink-0 text-progress-ink"
                        />
                    ...
                    <button
                        type="button"
                        onClick={onClear}
                        className="mt-2 inline-flex items-center gap-1 text-[12px] text-ink-muted hover:text-ink"
                    >
...
export function WallyDischarge() {
    ...
    const [dt, setDt] = useState<Doc>({
        uploaded: false,
        file: "Dietary_Care_Plan_MrTan.pdf",
        size: "192 KB",
    });
            ...
            <div className="mx-auto max-w-2xl px-5 py-8">
                ...
                <Card className="mx-auto max-w-md p-6 md:p-7">
                    <div className="mb-5 text-center">
                        <h1 className="text-[22px] font-bold text-accent-ink">
                            Upload your discharge plan
                        </h1>
                        <p className="mt-1.5 text-[14px] text-ink-muted">
                            Add the physiotherapy and dietitian care plans you
                            were given at discharge. Wally reads them to build
                            your personalised home recovery plan.
                        </p>
                    </div>
                    ...
                    <div className="space-y-3">
                        <UploadCard
                            ...
                            doc={pt}
                            onUpload={() =>
                                setPt((d) => ({ ...d, uploaded: true }))
                            }
                            onClear={() =>
                                setPt((d) => ({ ...d, uploaded: false }))
                            }
                        />
                        <UploadCard
                            icon={<Salad size={18} />}
                            ...
                            doc={dt}
                            onUpload={() =>
                                setDt((d) => ({ ...d, uploaded: true }))
                            }
                            onClear={() =>
                                setDt((d) => ({ ...d, uploaded: false }))
                            }
                        />
                    </div>

                    <p className="mt-4 text-center text-[12px] text-ink-subtle">
                        Your documents are private and shared only with your
                        care team.
                    </p>

                    {ready ? (
                        <a href="/preview/wally/review" className="mt-5 block">
