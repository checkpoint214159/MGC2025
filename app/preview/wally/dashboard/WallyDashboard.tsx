...
import {
    ...
    Flame, Beef, X, Sparkles, LifeBuoy,
} from "lucide-react";
...
function MacroBar({ icon, label, value, target, unit }: { icon: ReactNode; label: string; value: number; target: number; unit: string }) {
        ...
        <div>
            <div className="flex items-baseline justify-between">
                ...
                <span className="text-[13px] tabular-nums text-ink">
                    {Math.round(value)} <span className="text-ink-muted">/ {target} {unit}</span>
                </span>
            </div>
...
function NutritionCard({ logged, onAdd, onRemove }: { logged: DishId[]; onAdd: (d: DishId) => void; onRemove: (i: number) => void }) {
    ...
    const suggested: DishId[] = proteinLeft >= 30 ? ["fishSoup", "kwayChap"] : kcalLeft >= 450 ? ["meeSoto", "chickenRice"] : [];
    const suggestion =
    ...
    const sorted = [...SG_DISHES].sort((a, b) => (suggested.includes(b.id) ? 1 : 0) - (suggested.includes(a.id) ? 1 : 0));
    ...
    return (
        <SectionCard phase="assessment">
            <CardHead
                icon={<Salad size={18} />}
                title="Nutrition"
                chip={<Chip tone={kcal >= KCAL_TARGET && protein >= PROTEIN_TARGET ? "progress" : "accent"}>{kcal} / {KCAL_TARGET} kcal</Chip>}
            />

            <div className="space-y-2.5">
                <MacroBar icon={<Flame size={14} />} label="Calories" value={kcal} target={KCAL_TARGET} unit="kcal" />
                <MacroBar icon={<Beef size={14} />} label="Protein" value={protein} target={PROTEIN_TARGET} unit="g" />
            </div>
            ...
            {dishes.length > 0 && (
                <ul className="mt-3 space-y-1.5">
                    {dishes.map((d, i) => (
                        <li key={`${d.id}-${i}`} className="flex items-center gap-2.5 rounded-xl bg-accent-soft/30 p-1.5 pr-2">
                            <FoodIcon dish={d.id} size={38} />
                            <div className="min-w-0 flex-1 leading-tight">
                                <div className="truncate text-[14px] font-medium text-ink">{d.name}</div>
                                <div className="text-[12px] text-ink-muted">{d.kcal} kcal · {d.protein}g protein</div>
                            </div>
                            <button type="button" onClick={() => onRemove(i)} aria-label={`Remove ${d.name}`} className="grid size-8 shrink-0 place-items-center rounded-full text-ink-subtle hover:bg-surface-sunken hover:text-ink">
            ...
            <div className="mt-3 flex items-start gap-2 rounded-xl bg-accent-soft/40 p-3">
                <Sparkles size={15} className="mt-0.5 shrink-0 text-accent-ink" />
                <p className="text-[13px] leading-snug text-accent-ink">{suggestion}</p>
            </div>

            {/* add a meal — SG favourites */}
            <p className="mb-2 mt-3 text-[12px] font-semibold uppercase tracking-wide text-accent-ink">Log a meal · Singapore favourites</p>
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                ...
                                isSuggested ? "border-accent bg-accent-soft/30" : "border-border",
                            )}
                        >
                            <FoodIcon dish={d.id} size={42} />
                            <div className="min-w-0 flex-1 leading-tight">
                                {isSuggested && <div className="text-[10px] font-bold uppercase tracking-wide text-accent-ink">Wally suggests</div>}
                                <div className="truncate text-[13px] font-medium text-ink">{d.name}</div>
                                <div className="text-[11.5px] text-ink-muted">{d.kcal} kcal · {d.protein}g</div>
                            </div>
            ...
            <button type="button" onClick={() => setShowAll((s) => !s)} className="mt-2 w-full rounded-lg py-1.5 text-[13px] font-medium text-accent-ink hover:bg-surface-sunken">
                {showAll ? "Show fewer dishes" : `Show all ${SG_DISHES.length} dishes`}
            </button>
            <p className="mt-1.5 text-[11px] leading-snug text-ink-subtle">
                Per-serving estimates · singaporecalorie.com &amp; healthscreening.sg
            </p>
        </SectionCard>
    );
}
    ...
    return (
        <div className="mx-auto max-w-md px-4 py-6">
            ...
            {tab === "support" ? (
                ...
                <div className="mt-4 space-y-3.5">
                    ...
                    <NutritionCard
                        ...
                        onRemove={(i) => setLogged((l) => l.filter((_, idx) => idx !== i))}
                    />
            ...
            <nav className="sticky bottom-3 mt-4 flex items-center justify-around rounded-2xl border border-border bg-surface/95 px-2 py-2 shadow-md backdrop-blur">
                {[
                    { icon: <Home size={20} />, label: "Home", onClick: () => setTab("home"), active: tab === "home" },
                    { icon: <ClipboardList size={20} />, label: "Tasks" },
                    { icon: <BarChart3 size={20} />, label: "Progress" },
                    { icon: <LifeBuoy size={20} />, label: "Support", onClick: () => setTab("support"), active: tab === "support" },
                    ...
                    <button
                        ...
                        className={cn(
                            ...
                            t.onClick ? "hover:bg-surface-sunken" : "cursor-default",
                        )}
                    >
                        {t.icon}
                        {t.label}
