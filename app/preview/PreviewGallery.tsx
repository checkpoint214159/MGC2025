...
import {
    ...
    STAGE_LABEL,
    SPECIES_IDS,
} from "@/components/recovery/DailyPlant";
...
import type { Streak, Nudge } from "@/lib/engagement";
            ...
            <Section label="Flower varieties — randomly rotated each day (shown in bloom)">
                <Card className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                    {SPECIES_IDS.map((id) => (
                        <div
                            key={id}
                            className="flex flex-col items-center gap-1 rounded-md border border-border bg-surface p-2 text-center"
                        >
                            <PlantArt
                                stage={6}
                                species={id}
