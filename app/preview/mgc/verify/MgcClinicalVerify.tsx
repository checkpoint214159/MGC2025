...
const EXERCISE = [
  { name: "Ankle pumps", detail: "3 sets of 10, twice daily", intensity: "Easy" },
  { name: "Short hallway walk", detail: "3 min, twice daily", intensity: "Cautious" },
  { name: "Seated knee extensions", detail: "2 sets of 8", intensity: "Easy" },
];
const NUTRITION = [
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 px-5 py-10">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-[26px] font-semibold text-ink">Verify plan — Margaret Chen</h1>
          <p className="text-[15px] text-ink-muted">Open colectomy · 28-day recovery arc · drafted by the model</p>
        </div>
      ...
      {active ? (
        <Card className="space-y-2 border-progress/20 bg-progress-soft/30 p-8 text-center">
          ...
          <h2 className="text-[19px] font-semibold text-ink">Plan activated</h2>
          <p className="text-[15px] text-ink-muted">Both clinicians have signed off. Margaret&apos;s daily recovery loop is now live.</p>
          <a href="/preview/mgc/patient" className="inline-block pt-2">
        ...
        <>
          ...
          <Card className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[13px] text-ink-subtle">your discipline</span>
            </div>
            <ul className="divide-y divide-border rounded-md border border-border">
              {(role === "pt" ? EXERCISE : NUTRITION).map((item) => (
                <li key={item.name} className="flex items-center gap-3 px-4 py-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[13px] text-ink-muted">{item.detail}</div>
                  </div>
                  {"intensity" in item && <Chip tone="accent" size="sm">{(item as { intensity: string }).intensity}</Chip>}
                  <button aria-label="Edit" className="grid size-9 place-items-center rounded-md text-ink-subtle hover:bg-surface-sunken hover:text-ink">
