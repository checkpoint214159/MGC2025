  ...
  try {
    ...
    if (dbConfig) {
      const config: StateGenerationConfig = {
        contextWindowDays:
          parseInt(process.env.CONTEXT_WINDOW_DAYS ?? String(dbConfig.contextWindowDays)) ||
          DEFAULT_CONFIG.contextWindowDays,
        smartFiltering:
          process.env.SMART_FILTERING === "true" || dbConfig.smartFiltering,
        maxContextTokens:
          parseInt(process.env.MAX_CONTEXT_TOKENS ?? String(dbConfig.maxContextTokens)) ||
          DEFAULT_CONFIG.maxContextTokens,
        includeTrajectory:
          process.env.INCLUDE_TRAJECTORY === "true" || dbConfig.includeTrajectory,
      };
    ...
    console.warn("Failed to load config from DB, falling back to env vars:", e);
  }
...
export async function updateStateGenerationConfig(
  ...
  await prisma.graphConfig.upsert({
    ...
    create: {
      ...
      contextWindowDays: updates.contextWindowDays ?? DEFAULT_CONFIG.contextWindowDays,
      smartFiltering: updates.smartFiltering ?? DEFAULT_CONFIG.smartFiltering,
      maxContextTokens: updates.maxContextTokens ?? DEFAULT_CONFIG.maxContextTokens,
      includeTrajectory: updates.includeTrajectory ?? DEFAULT_CONFIG.includeTrajectory,
    },
    update: {
...
export async function resetStateGenerationConfig(): Promise<StateGenerationConfig> {
  await prisma.graphConfig.deleteMany({
    where: { graphName: "state-generation" },
  });
