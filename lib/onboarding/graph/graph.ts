...
async function buildGraph() {
    ...
    return (
        new StateGraph(OnboardingAnnotation)
            // --- Nodes ---
            .addNode(
                "load_biometrics",
                loggedNode("load_biometrics", loadBiometricsNode),
            )
            .addNode(
                "collect_screening_responses",
                loggedNode(
                    "collect_screening_responses",
                    collectScreeningResponsesNode,
                ),
            )
            .addNode(
                "evaluate_screening",
                loggedNode("evaluate_screening", evaluateScreeningNode),
            )
            .addNode(
                "generate_question",
                loggedNode("generate_question", generateQuestionNode),
            )
            .addNode(
                "interrupt_question",
                loggedNode("interrupt_question", interruptQuestionNode),
            )
            .addNode(
                "generate_profile",
                loggedNode("generate_profile", generateProfileNode),
            )
            .addNode(
                "save_profile",
                loggedNode("save_profile", saveProfileNode),
            )
