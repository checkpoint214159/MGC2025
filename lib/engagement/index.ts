...
import {
    ...
    ExercisePlan,
    ExerciseMetrics,
} from "@/lib/state/schemas/exercise";
...
export function getRecoveryDay(opts: {
...
function exercisePriorities(mod: ExerciseModule): Priority[] {
    ...
    return mod.plan.map((p: ExercisePlan) => {
        ...
        const progressMetrics = trackable
            ? (Object.values(trackable.data ?? {}) as ExerciseMetrics[])
            : [];
        const totalGoal = planMetrics.reduce((s, m) => s + (m?.goal ?? 0), 0);
        const totalValue = progressMetrics.reduce(
            (s, m) => s + (m?.value ?? 0),
            0,
        );
...
function nutritionPriorities(mod: NutritionModule): Priority[] {
    if (!mod.plan?.length) return [];
    const macroPlan = mod.plan.find(
        (p: NutritionPlan) => p.meta?.type === "macros",
    ) as NutritionPlan | undefined;
    if (!macroPlan) return [];
    const trackable = mod.progress?.trackables.find(
        (t) => t.id === macroPlan.id,
    );
    ...
    if (goalCalories > 0) {
        items.push({
            ...
            context: `${Math.round(valCalories)} / ${Math.round(
                goalCalories,
            )} kcal`,
    ...
    if (proteinGoal > 0) {
        items.push({
            ...
            context: `${Math.round(proteinVal)} / ${Math.round(
                proteinGoal,
            )} g — for tissue repair`,
...
export function getWeekRhythm(
    ...
    const todayDone =
        priorities.length > 0 && priorities.every((p) => p.isComplete);
    return {
        completed: todayDone ? todayIndex + 1 : todayIndex,
        total: 7,
        todayIndex,
    };
}
