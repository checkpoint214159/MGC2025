...
export function getCompletionSignal(
    series: DayProgress[],
    threshold = NEAR_COMPLETE_PCT,
): { near: boolean; latest: number | null } {
...
export function getProgressStagnation(
    series: DayProgress[],
    window = STALL_WINDOW,
): {
    stalled: boolean;
    ...
    return {
        stalled: to < 100 && to - from < MIN_WEEKLY_GAIN,
        window,
        from,
        to,
    };
...
export function getProgressRegression(
    series: DayProgress[],
    days = DROP_DAYS,
): { dropping: boolean; days: number; from: number | null; to: number | null } {
    if (series.length < days + 1)
        return { dropping: false, days, from: null, to: null };
    let drops = 0;
