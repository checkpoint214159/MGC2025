...
const p = (...vals: number[]): DayProgress[] => vals.map((progress, i) => ({ day: i + 1, progress }));
...
describe("getProgressStagnation (7-day flat)", () => {
  it("flags when progress barely moves over the window", () => {
    expect(getProgressStagnation(p(55, 55, 56, 55, 56, 55, 56)).stalled).toBe(true);
  });
  it("does not flag when progress is climbing", () => {
    expect(getProgressStagnation(p(40, 44, 48, 52, 56, 60, 64)).stalled).toBe(false);
  });
  it("does not flag once complete", () => {
    expect(getProgressStagnation(p(100, 100, 100, 100, 100, 100, 100)).stalled).toBe(false);
  });
...
describe("getProgressRegression (2-day drop)", () => {
  it("flags two consecutive day-over-day declines", () => {
    expect(getProgressRegression(p(40, 50, 48, 45).map((d) => d)).dropping).toBe(true);
  });
...
describe("evaluateRecoveryFlags", () => {
  ...
  it("orders critical before info", () => {
    const flags = evaluateRecoveryFlags({
      ...
      pain: [{ day: 6, pain: 6 }, { day: 7, pain: 6 }, { day: 8, pain: 6 }],
    });
