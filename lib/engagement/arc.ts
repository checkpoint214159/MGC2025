...
export type ArcResult = { points: ArcPoint[]; deltaToday: number | null; onTrack: boolean };
export type Stagnation = { stalled: boolean; window: number; from: number | null; to: number | null };

const PAIN_FLOOR = 1; // pain at/below this is "settled" — never flag.

const recoveryDayOf = (dateCreated: Date | string, surgeryDate: Date): number => {
...
export function getStatePain(state: State): number | null {
  ...
  const trackables = (sym?.progress?.trackables ?? []) as Array<{ data?: Record<string, { value?: number; type?: string }> }>;
  for (const t of trackables) {
    const data = t.data ?? {};
    const entry = data.pain ?? Object.values(data).find((m) => m?.type === "pain");
...
export function getPainSeries(states: State[], surgeryDate: Date): DayPain[] {
  return states
    .map((s) => {
      ...
      return pain === null ? null : { day: recoveryDayOf(s.dateCreated, surgeryDate), pain };
    })
...
export function getActualVsExpected(series: DayPain[], recoveryDays: number, baselinePain: number): ArcResult {
  const actualByDay = new Map(series.map((d) => [d.day, d.pain]));
  const points: ArcPoint[] = [];
  for (let day = 1; day <= recoveryDays; day++) {
    points.push({
      day,
      expected: getExpectedRecovery({ recoveryDay: day, recoveryDays, baselinePain }),
      actual: actualByDay.get(day) ?? null,
    });
  }
  const today = series.length ? series[series.length - 1] : null;
  const deltaToday = today
    ? today.pain - getExpectedRecovery({ recoveryDay: today.day, recoveryDays, baselinePain })
    : null;
