...
import { getExpectedRecovery, getActualVsExpected, getStagnationSignal, type DayPain } from "@/lib/engagement/arc";

describe("getExpectedRecovery", () => {
  it("starts at baseline on day 1", () => {
    expect(getExpectedRecovery({ recoveryDay: 1, recoveryDays: 21, baselinePain: 8 })).toBeCloseTo(8, 5);
  });
  it("reaches ~0 at the end of the arc", () => {
    expect(getExpectedRecovery({ recoveryDay: 21, recoveryDays: 21, baselinePain: 8 })).toBeCloseTo(0, 5);
  });
  it("decreases monotonically", () => {
    const a = getExpectedRecovery({ recoveryDay: 5, recoveryDays: 21, baselinePain: 8 });
    const b = getExpectedRecovery({ recoveryDay: 10, recoveryDays: 21, baselinePain: 8 });
...
describe("getStagnationSignal (the flag rule)", () => {
  const s = (...vals: number[]): DayPain[] => vals.map((pain, i) => ({ day: i + 1, pain }));
