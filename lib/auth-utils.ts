...
export async function requireRole(requiredRole: 'patient' | 'admin'): Promise<string> {
  ...
  if (userRole !== requiredRole) {
    throw new Error(`Unauthorized: user role '${userRole}' does not match required role '${requiredRole}'`);
  }
 ...
 */
export async function requirePatientAccess(userId: string, targetPatientId: string): Promise<void> {
  ...
  if (!adminRelation) {
    throw new Error(`Forbidden: user ${userId} does not have access to patient ${targetPatientId}`);
  }
}
 ...
 */
export async function getAdminManagedPatientIds(adminId: string): Promise<string[]> {
