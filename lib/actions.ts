...
export async function authenticatedAction<T>(
    ...
    if (!userId) {
        throw new Error(
            "Unauthorized user attempted to call an action. This incident will be reported to ben.",
        ); // no, it wont
    }
...
export async function deleteOnboardingDataAction() {
    authenticatedAction(async (userId) => {
...
export async function compileExternalAction(
...
export async function fetchStateAction(
    ...
    return await authenticatedAction(async (userId): Promise<State> => {
        console.log(
            "[fetchStateAction] 🚀 Starting - userId:",
            userId,
            "date:",
            date,
            "admin_force:",
        ...
        if (!state || admin_force) {
            console.log(
                "[fetchStateAction] ⚠️ No state found, calling generateNewState",
            );
            ...
            console.log(
                "[fetchStateAction] ✅ State already exists, returning",
            );
        }
        return state;
    });
...
export async function fetchRecoveryHistoryAction() {
    // Read-only: the full active-State chain plus the patient's surgery date, used to
    // derive streak / recovery arc / progress chart. Never triggers state generation.
    return await authenticatedAction(async (userId) => {
        const [states, bio] = await Promise.all([
        ...
        return { states, surgeryDate: bio?.surgeryDate ?? null };
    });
}
 ...
 */
export async function assignPatientToAdminAction(
    patientId: string,
    adminId: string,
) {
