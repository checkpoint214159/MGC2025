...
```json
{ "inputType": "text", "options": [], "metadata": { "intent": "environment", "urgency": false, "sliderMin": 0, "sliderMax": 10 } }
```
...
2. **Drives the browser** or calls server actions directly, answering each onboarding question *in character*
...
const bio = { age: persona.age, sex: persona.sex, treatment: persona.surgery, surgeryDate: new Date() };
...
const profile = await generateUserProfile({ thread, biometrics: bio, baseline });
